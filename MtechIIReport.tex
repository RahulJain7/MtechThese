...
\begin{figure}[t]
\centering
\includegraphics[width=1\linewidth]{DP1}
...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{VLE4}
...
\subsection{The Model}
In this model a steady state flash was simulated in openmodelica using the developed thermodynamics. The components involved are Benzene and Toluene and as this system is an ideal system, the thermodynamic package used is Raoultâ  Law.The feed is equimolar mixture of benzene and toluene. To check the design efficiency of the developed thermodynamic engine in openmodelica , the output composition of the vapor product was specified, while the temperature at which this desired composition is attained is kept unknown. To carry out this simulation in DWSSIM, we have to use an adjust operation which uses a trial and error method and is time consuming. The flash algorithm involved is PT flash. The example is from the book Simultaneous modular simulation and optimization \cite{Beigler}
