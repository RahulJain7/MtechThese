...
Before simulating a model, OpenModelica instanciate it by itself. It is at this stage that OpenModelica's compiler collects all the equations and inserts them in the \textbf{equation section} of the instantiated model. Figure \ref{8.3} shows the instance of the flash model as generated by OpenModelica. It is clear that now this model involves both the modeling equations and the thermodynamic equations. 
\begin{figure}
\centering
\includegraphics[width=1\linewidth]{Combmodel}
\caption{Combined model generated by OpenModelica}
\label{8.3}
\end{figure}

After the instance of the main model is generated, OpenModelica start solving all the equations present in the \textbf{equation} section of the instantiated model. By default OpenModelica uses \textbf{Dassl} to solve differential algebric equations and a hybrid solver for solving algebraic equations. However, we can choose a different solver from the list of solvers. This is achieved by going in the simulation setup window, where one can specify all the details regarding the simulation for example the time period, the number of intervals for simulation etc. The output of OpenModelica's successful simulation is a mat or a csv file.
...
The pressure is kept constant at 1 atm. The value of $y_1$ (mole fraction of ethanol) is varied from 0 to 1 with an interval of 0.1 and for each value of $y_1$ the corresponding value of temperature if calculated by equation \ref{eqn:RL4}. Figure \ref{fig:DP} shows the resulting T-y curve as calculated by the developed built in thermodynamics in OpenModelica.

\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{DP}
\caption{Dew point (T-y) curve for ethanol water system generated in OpenModelica.}
\label{fig:DP}
\end{figure}

\item {\textbf{VLE curve}} \\ 
Combining the bubble point and the dew point curves as generated above gives the T-xy curve for the system as shown in Figure \figref{fig:VLE}

This figure is verified by reproducing the same problem in Aspen plus. Figure \figref{aspen} depicts the T-xy curve for ethanol water system generated in Aspen plus. It clearly shows that the plot generated in OpenModelica completely replicates the one generated in Aspen plus. 

\begin{figure}
\centering
\subfigure[Results from OpenModelica]{
\includegraphics[width=0.9\linewidth]{VLE}
\label{fig:VLE}}
\subfigure[Results from Aspen Plus]{
\includegraphics[width=0.9\linewidth]{aspen}
\label{aspen}}
\caption{Comparison of T-xy curve for ethanol water system using UNIQUAC VLE model}
\end{figure}
...
\label{SD1}}
\subfigure[Results from reference \cite{PHD}]{
\includegraphics[width=0.8\linewidth]{SD4}
\label{SD4}}
\caption{Comparison of change in organic phase composition during semi-batch steam distillation}
\label{composition}
\end{figure}
...
\label{SD2}}
\subfigure[Results from reference \cite{PHD}]{
\includegraphics[width=0.8\linewidth]{SD3}
\label{SD3}}
\caption{Comparison of temperature change during semi-batch steam distillation}
\label{temperature}
\end{figure}

Figures \ref{composition} and \ref{temperature} shows that the results of OpenModelica are in agreement with the reference \cite{PHD}. Therefore, the built in thermodynamics can be claimed to be robust and accurate.
...
\subsection{Results}
Figure \ref{ingcomp} shows the composition of methanol increases rapidly at the start as there is total reflux initially and then increases at a lower rate as now the reflux ratio is 10. The reboiler is slowly getting leaner in methanol as the holdup in roboiler is very high as compared to the trays. 

\begin{figure}
\centering
\subfigure[Results from OpenModelica]{
\includegraphics[width=1\linewidth]{Inghamb}
\label{Ingham4}}
\subfigure[Results from reference \cite{ingham}]{
\includegraphics[width=1\linewidth]{Ingham2}
\label{Ingham2}}
\caption{Comparison of methanol composition profiles of liquid holdup in reboiler and vapour leaving form tray 1}
\label{ingcomp}
\end{figure}


Figure \ref{ingtemp} shows that the temperature of all the trays drastically decreases and becomes less than the reboiler. The reboiler temperature changes slowly because the holdup is very high as compared to trays. One can also notice that as we move upwards along the column, the temperature decreases, this is due to the fact that the as we move upwards along the column the composition gets richer in more volatile component.

\begin{figure}
\centering
\subfigure[Results from OpenModelica]{
\includegraphics[width=1\linewidth]{Inghama}
\label{Ingham5}}
\subfigure[Results from reference \cite{ingham}]{
\includegraphics[width=1\linewidth]{Ingham3}
\label{Ingham3}}
\caption{Comparison of temperature profiles of tray 1,2,3,4 and reboiler with time}
\label{ingtemp}
\end{figure}
